...
#@pytest.mark.skip("TODO")
def test_empty():
    '''We never add the node to the graph. This is why they test the graph and the graph should be empty.'''
    graph = Graph()
...
def test_island_empty():
    '''Here we are using the graph class in the next line.'''
    graph = Graph()
...
def test_island_crowded(graph):
    '''Here we are PASSING IN the graph from the fixture.'''
    lonely = graph.add_node("lonely")
    actual = graph.depth_first_search(lonely)
    expected = ["lonely"]
    assert actual == expected

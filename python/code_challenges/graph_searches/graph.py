...
class Graph:
    ...
    def breadth_first_search(self, root):
        ...
        while queue.front is not None:
            if queue.front.value not in visited:
                visited.append(queue.front.value)
                values.append(queue.front.value.value)

                neighbors = self.get_neighbors(queue.front.value)
    ...
    def depth_first_search(self, root):
        # This version passes tests 3 and 4
        stack = Stack()
        ...
        # Incorporate for test 2
        # if root not in self.get_nodes():
        #     return values

        while stack.top is not None:
            if stack.top.value not in visited:
                visited.append(stack.top.value)
                print(stack.top.value)
                values.append(stack.top.value.value)

                neighbors = self.get_neighbors(stack.top.value)
                for node in neighbors:
                    stack.push(node)
            stack.pop()
